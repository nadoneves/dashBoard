/**
* @preserve CanvasJS HTML5 & JavaScript Charts - v1.6.0 Beta- http://canvasjs.com/ 
...
(function () {
	...
	Chart.prototype.render = function (options) {
        ...
        if (this.animatedRender) {
            ...
            //document.getElementById("chartContainer2").appendChild(initialState);
        ...
        if (this.animatedRender && this._indexLabels.length > 0) {
            ...
            //this.renderIndexLabels(indexLabelCanvasCtx);
        ...
        if (animations.length > 0) {
            ...
            this._animator.animate(this.animationDuration, function (fractionComplete) {
                ...
                for (var l = 0; l < animations.length; l++) {
                    ...
                    //    document.getElementById("chartContainer2").appendChild(animationInfo.source.canvas);
                    ...
                    if (fractionComplete < 1 && typeof (animationInfo.startTimePercent) !== "undefined") {
                        ...
                        //animationInfo.animationCallback(AnimationHelper.easing.linear(fractionComplete, 0, 1, 1), animationInfo);
	...
	Chart.prototype.renderPie = function (plotUnit) {
		...
		this.pieDoughnutClickHandler = function (e) {
            ...
            if (dataSeries.dataPoints.length > 1) {
                _this._animator.animate(explodeDuration, function (fractionComplete) {
        ...
        this.disableToolTip = true;
        this._animator.animate(this.animatedRender ? this.animationDuration : 0, function (fractionComplete) {
            ...
            _this.disableToolTip = false;
            _this._animator.animate(_this.animatedRender ? explodeDuration : 0, function (fractionComplete) {
		...
		for (var i = 0; i < this.dataSeries.length; i++) {
			...
			if (dataSeries.type !== "pie" && dataSeries.type !== "doughnut" && dataSeries.type !== "funnel") {
				var item = {
                    ...
                    dataSeriesIndex: dataSeries.index, dataPointIndex: null
				};
	...
	Axis.prototype.calculateAxisParameters = function () {
		...
		if (this.type === "axisX") {
			...
			if (max - min === 0) {
                ...
                var rangePadding = Math.min(Math.abs(Math.abs(max - min) * .01), .05);
				if (max !== 0)
                    max += rangePadding;
				if (min !== 0)
                    min -= rangePadding;
    ...
    Animator.prototype.animate = function (duration, animationCallback, onComplete, easingFunction) {
        var _this = this;
        ...
        while (this.animations.length > 0) {
            ...
            var fractionComplete = easingFunction(Math.min(((new Date()).getTime() - animation.startTime), animation.duration), 0, 1, animation.duration);
    ...
    CanvasJS.Chart.version = "v1.6.0 Beta";
